...
public class DocumentGoodsIssued  extends AbstractXlsView{
	 //https://www.codeleading.com/article/53651723235/

	@Override
	protected void buildExcelDocument(Map<String, Object> model, Workbook workbook, HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		// TODO Auto-generated method stub
		StringBuilder fileName = new StringBuilder("document_Xuat-Hang_.xls");
		...
		row.createCell(4).setCellValue("Khách hàng");
		...
		List<CustomerDTO> listCus = (List<CustomerDTO>) model.get("customers") ;
		int cusSize = listCus.size();
		String[] arrayCus = new String[cusSize];
		for(int i = 0 ; i < cusSize ;i++  ) {
			arrayCus[i] = listCus.get(i).getName();
		}	
		...
		dataValidation  = dropDown( workbook,1 , 4, arrayCus);
		sheet.addValidationData(dataValidation);
