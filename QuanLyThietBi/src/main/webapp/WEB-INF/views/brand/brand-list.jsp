...
<div class="right_col" role="main">
					...
					<div class="row">
						<div class="col-md-12 col-sm-12 ">
							<div class="x_panel">
								<div class="x_content">
									<br />
									<c:url value="/brand/list/1" var="searchUrl"></c:url>
	...
	<div class="table-responsive">
		<a href='<c:url value="/brand/add"></c:url>'><button class="btn btn-primary"><i class="glyphicon glyphicon-plus"></i>Thêm</button></a>
		<!-- 
		...
		<a href='<c:url value="/brand/excel-file"></c:url>'><button class="btn btn-default" title="lấy mẫu import"><i class="glyphicon glyphicon-file"></i> Document</button></a>
		 -->
                        ...
                        <tbody>
                          <c:forEach items="${list}" var="item" varStatus="i"> 
                            ...
                            <td colspan="3" class="last text-center">
	                            ...
	                            <a href='<c:url value="/brand/view/${item.id}"></c:url>' class="btn btn-primary"><i class="glyphicon glyphicon-eye-open"></i></a> 
	                            <a href='<c:url value="/brand/edit/${item.id}"></c:url>' class="btn btn-warning"><i class="glyphicon glyphicon-edit"></i></a> 
		...
		<div id="excel-modal" class="modal fade">
			<div class="modal-dialog">
				<div class="modal-content">
				  <form action='<c:url value="/manage/brand/import-excel"></c:url>' method="post" enctype="multipart/form-data">
...
<script type="text/javascript">
	function gotoPage(page){
		$("#searchForm").attr('action',"<c:url value='/brand/list/'/>"+page);
		$("#searchForm").submit();
	}
	function deleteItem(id){
		if(confirm("Bạn có chắc chắn xóa nó không ?")){
			location.href="<c:url value='/brand/delete/'/>"+id;
		}
	}
	...
	$(document).ready(function(){
		$('#brandlist').parents("li").addClass('active').siblings().removeClass("active");
		$('#brandlist').addClass('current-page').siblings().removeClass("current-page");
		$('#brandlist').parents().show();
	});
